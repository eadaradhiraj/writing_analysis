...
class FeatureExtractor:
    ...
    def yule_i(self, sent_spans_index_start, sent_spans_index_end):
        m1 = 0.0
        m2 = 0.0
        for counts in list(self.__wordcount(sent_spans_index_start, sent_spans_index_end).values()):
            m1 += counts
            m2 += counts ** 2
        try:
            return (m1*m1)/(m2-m1)
    ...
    def hapax_dislegomena(self, sent_spans_index_start, sent_spans_index_end):
        ...
        text = self.text[start:end]
        wds = nltk.tokenize.word_tokenize(text)
        twice_seen_wds = set([wd for wd in wds if wds.count(wd) == 2])
    ...
    def alpha_chars_ratio(self, sent_spans_index_start, sent_spans_index_end):
        ...
        for each_char in text:
            if each_char.isalpha():
                alpha_count+=1
        return float(alpha_count/len(text))
    ...
    def digit_chars_ratio(self, sent_spans_index_start, sent_spans_index_end):
        ...
        for each_char in text:
            if each_char.isdigit():
                digit_count+=1
        return float(digit_count/len(text))
    ...
    def upper_chars_ratio(self, sent_spans_index_start, sent_spans_index_end):
        ...
        for each_char in text:
            if each_char.isupper():
                upper_char_count+=1
        return float(upper_char_count/len(text))
    ...
    def white_chars_ratio(self, sent_spans_index_start, sent_spans_index_end):
        ...
        for each_char in text:
            if each_char.isspace():
                white_char_count+=1
        return float(white_char_count/len(text))
    ...
    def __calcv(self, sent_spans_index_start, sent_spans_index_end):
        ...
        total = 0.0
        for counts in wordcounts.values():
            freqs_seen[counts] += 1
            total += counts
        V = {}
        for count, seen in freqs_seen.items():
            V[count] = seen / total
        return V
    #new
    ...
    def dale_chall_readability_score(self, sent_spans_index_start, sent_spans_index_end):
        wordcount = float(sum(self.__wordcount(sent_spans_index_start, sent_spans_index_end).values()))
    ...
    def smog_index(self, para_spans_index_start, para_spans_index_end):
        ...
        if len(nltk.tokenize.word_tokenize(text)) >= 0:
            try:
                ps = self.polysyllablcount(para_spans_index_start, para_spans_index_end)
                smog = (1.043 * (30*(ps/len(nltk.sent_tokenize(text))))**.5) + 3.1291
                return round(smog,1)
            except:
    ...
    def _init_avg_internal_word_freq_class(self):
        ...
        corpus_word_freq_by_rank = sorted(list(word_freq_dict.items()), key=lambda x: x[1], reverse=True)
    ...
    def _init_avg_external_word_freq_class(self):
        ...
        #    word = word.lower()
        #    word_freq_dict[word] = word_freq_dict.get(word, 0) + 1
        word_freq_dict = pickle.load(open(os.path.join(os.path.dirname(__file__), "word_freq.pkl",), "rb"))

...
def get_confidences1(stylo_vectors, centroids, cluster_assignments):
	...
	list_nonplag_tuples = []
	list_plag_tuples = []
	confidences = [0 for z in range(len(stylo_vectors))]
	list_indicies = [z for z in range(len(stylo_vectors))]
	list_abs_differences = [0 for q in range(len(stylo_vectors))]
	'''
	...
	for i in range(len(stylo_vectors)):
	...
	if (len(list_plag_tuples) != 0) :
		...
		for x in range(len(list_plag_tuples)) :
	...
	if (len(list_nonplag_tuples) != 0) :
		...
		for y in range(len(list_nonplag_tuples)) :
...
def get_confidences2(stylo_vectors, centroids, cluster_assignments):
	...
	confidences = [0 for z in range(len(stylo_vectors))]
	list_indicies = [z for z in range(len(stylo_vectors))]
	list_abs_difference_from_assign = [0 for q in range(len(stylo_vectors))]
	list_abs_difference_from_other = [0 for q in range(len(stylo_vectors))]
...
def hmm_cluster(stylo_vectors, k):
	...
	means_sum = [0 for j in range(len(stylo_vectors[0]))]
	for vector in stylo_vectors:
		for i in range(len(vector)):
			means_sum[i] += vector[i]
	for i in range(len(means_sum)):
		means_sum[i] /= float(len(stylo_vectors))
	
	variances = [0.0000001 for j in range(len(stylo_vectors[0]))]
	varianceSmall = [0.0000001 for j in range(len(stylo_vectors[0]))]
	varianceLarge = [0.0000001 for j in range(len(stylo_vectors[0]))]
	...
	clusters_indices = {i: [] for i in range(k)}
	for i in range(len(trained_path)):
		clusters_indices[trained_path[i]].append(i)

	# calculate centroids of each cluster
	centroids = []
	for i in range(k):
		centroid = [0.0 for x in range(len(stylo_vectors[0]))]
...
def train_parameters(feature_vectors, states, initial_state_probs):
	...
	while percent_change > 0.01:
		num_states = [viterbi_path.count(i) for i in range(len(states))]

		# calculate means of observed outputs with each state
		means_sum = [[0 for j in range(len(feature_vectors[0]))] for i in range(len(states))]
		index = 0
		...
		variances_sum = [[0 for j in range(len(feature_vectors[0]))] for i in range(len(states))]
		index = 0
		for state in viterbi_path:
...
def viterbi(feature_vectors, states, initial_state_probs):
	...
	table = [[-float('inf') for j in range(len(states))] for i in range(len(feature_vectors))]
	table2 = [[-1 for j in range(len(states))] for i in range(len(feature_vectors))]

...
def get_plagiarism(text, atom_type, features, cluster_method, k):
    ...
    # Return it
    return list(zip(tokenize(text, atom_type), confidences)) # should feature extractor have a method that returns the spans it used instead?
...
def get_plagiarism_passages(text, atom_type, features, cluster_method='none', k=2):
    ...
    feature_vecs = [list(p.features.values()) for p in passages]
